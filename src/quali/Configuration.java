...
public class Configuration {
    ...
    public int unary = -1;// 0 = regular, 1 = slim lean to right, 2 = slim lean
			  ...
			  // left , -1 = not initialized, -2 = completed.
    ...
    public int lastValidNeighborId = -2; // id = -2, touches all others -1, no
					 // neighbors
	...
	if (!tconf.getMbr().equals(mbr)) {
	    ...
	    Contact contact = contactManager.getPairContact(tconf, this,
		    WorldinVision.gap);
		...
		// Debug.echo(this,
		// this.contact_map.size(),contact,contact.getType());
		if (contact.getType() == 1) {
			...
			if (tangential_area == 23 || tangential_area == 3) {
				...
				left_support++;
			} else if (tangential_area == 4
				|| tangential_area == 34) {
			    right_support++;
			    weaksupport = true;
			    if (contact.strongEdgeSupport)
				right_support++;
			} else if (tangential_area == 12) {
			    if (unary > 2) {
				weaksupport = true;
				left_support++;
			    }
			} else if (tangential_area == 14) {
			    if (unary > 2) {
				weaksupport = true;
				right_support++;
			    }
	...
	if (height > width) {
	    ...
	    limit_vertical = height
		    / 2
		    - (int) Math.sqrt((height / 2) * (height / 2) - (width / 2)
			    * (width / 2));
	} else {

	    limit_vertical = height / 2;
	    limit_horizontal = width
		    / 2
		    - (int) Math.sqrt((width / 2) * (width / 2) - (height / 2)
			    * (height / 2));
	}
	 ...
	 * core_right2.addPoint(( x + limit_horizontal+ x)/2, ( y + height + y +
	 * height - limit_vertical)/2); core_right2.addPoint(( x + width + x +
	 * width - limit_horizontal)/2, ( y + limit_vertical+ y)/2);
	 * core_right2.addPoint( x + width - limit_horizontal, y);
	 * 
	 * 
	 * 
	 * core_right4.addPoint(( x + width + x + width - limit_horizontal)/2, (
	 * y + limit_vertical+ y)/2); core_right4.addPoint( x +
	 * limit_horizontal, y + height); core_right4.addPoint( x + width, y +
	 * limit_vertical); core_right4.addPoint(( x + width + x + width -
	 * limit_horizontal)/2, ( y + limit_vertical+ y)/2);
	 */
	...
	/*
	 * core_left1.addPoint( x + limit_horizontal, y); core_left1.addPoint((
	 * x + x + limit_horizontal)/2, ( y + limit_vertical + y)/2);
	 * core_left1.addPoint(( x + width - limit_horizontal+ x + width)/2, ( y
	 * + height+ y + height - limit_vertical)/2); core_left1.addPoint( x +
	 * width, y + height - limit_vertical);
	 * 
	 * core_left3.addPoint(( x + x + limit_horizontal)/2, ( y +
	 * limit_vertical + y)/2); core_left3.addPoint( x, y + limit_vertical);
	 * core_left3.addPoint( x + width - limit_horizontal, y + height);
	 * core_left3.addPoint(( x + width - limit_horizontal+ x + width)/2, ( y
	 * + height+ y + height - limit_vertical)/2);
	 */
    ...
    public boolean addRestrictedPoints(Point point, int region,
	    boolean contacted) {
	boolean result = true;
	if (region == 2) {
	    ...
	    /*
	     * first examine whether there are some shorter blocks supporting
	     * the object at left
	     */
		...
		points3.add(point);
	    /*
	     * then examine whether there are some higher blocks supporting the
	     * object at right
	     */
	    if (result && contacted) {
	...
	if (region == 1) {
		...
		result.addPoint((getX() + getWidth() / 2),
			(getY() + getHeight()));
	    } else {
		result.addPoint(x, y + height);
    ...
    public void setBlocked_regions(
	    HashMap<Integer, LinkedList<MBR>> blocked_regions) {
